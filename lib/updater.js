...
// Checks on launch and every few hours, downloads a newer release in the
// background, and once it's ready pushes a `update:status` event so the
// renderer can show a "restart to apply" banner. The install itself happens on
// the next quit (or immediately if the user clicks restart). Unsigned build:
// electron-updater verifies the download against the sha512 in latest.yml, so
// it works without code signing — it just skips the publisher-signature check.
...
module.exports = { initAutoUpdater, getUpdateState, checkForUpdates, quitAndInstall };
