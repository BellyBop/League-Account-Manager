...
// Once the champion list is in, keep re-checking on this slower cadence until
// the wallet shows up too (it can lag the champion list by a poll or three
// after a sign-in), then give up after WALLET_CHASE_ATTEMPTS so a client that
// genuinely never exposes it settles down.
const INVENTORY_WALLET_CHASE_MS = 12 * 1000; // < the renderer's poll interval, so every poll retries
const WALLET_CHASE_ATTEMPTS = 12;
let inventoryCache = { puuid: null, at: 0, value: null, attempts: 0 };

function hasChampions(v) {
  return Boolean(v && Array.isArray(v.ownedChampionIds) && v.ownedChampionIds.length);
}
function hasWallet(v) {
  return Boolean(v && v.blueEssence != null);
}
// Fully done — stop re-reading until the long TTL. Either we got everything, or
// the champion list is in and we've given the wallet a fair number of tries.
function inventorySettled(v, attempts) {
  return hasChampions(v) && (hasWallet(v) || attempts >= WALLET_CHASE_ATTEMPTS);
}
...
async function fetchInventory(lockfile, summonerId, walletOnly) {
  console.log(`[lcu] fetchInventory summonerId=${summonerId || '(missing)'}${walletOnly ? ' (wallet only)' : ''}`);
  ...
  if (typed && typeof typed === 'object') {
    ...
    if (be != null || rp != null) {
      ...
      console.log(`[lcu] wallet 200 but no BE/RP keys: ${JSON.stringify(typed).slice(0, 200)}`);
    }
  }
  ...
  // When we already have the champion/skin lists cached and are only chasing a
  // late-arriving wallet, skip re-pulling the big champion + 2500-entry skin
  // responses every poll.
  if (walletOnly) {
    console.log(`[lcu] fetchInventory result (wallet only): BE=${blueEssence} RP=${riotPoints}`);
    if (blueEssence == null && riotPoints == null) return null;
    return { blueEssence, riotPoints, ownedChampionIds: null, ownedSkins: null, fetchedAt: Date.now() };
  }
  ...
  console.log(
    `[lcu] fetchInventory result: BE=${blueEssence} RP=${riotPoints} ` +
    `champs=${ownedChampionIds ? ownedChampionIds.length : 'null'} skins=${ownedSkins ? ownedSkins.length : 'null'}`
  );
...
async function getAccountInventory(lockfile, summoner, force) {
  ...
  const cached = sameAcct ? inventoryCache.value : null;
  const attempts = sameAcct ? inventoryCache.attempts : 0;

  const chasingWallet = hasChampions(cached) && !inventorySettled(cached, attempts);
  let ttl;
  if (inventorySettled(cached, attempts)) {
    ttl = INVENTORY_TTL_MS;
  } else if (chasingWallet) {
    ttl = INVENTORY_WALLET_CHASE_MS; // champs shown; still chasing the wallet
  } else {
    ttl = Math.min(INVENTORY_RETRY_BASE_MS * 2 ** attempts, INVENTORY_RETRY_MAX_MS);
  }
  if (!force && sameAcct && now - inventoryCache.at < ttl) {
    return inventoryCache.value;
  }

  const fresh = await fetchInventory(lockfile, summoner && summoner.summonerId, chasingWallet && !force);
  const value = mergeInventory(cached, fresh);
  // A forced check (↻ button / sign-in) resets the attempt counter so the
  // wallet gets a fresh run of tries. Only a truly complete capture resets it
  // back to 0 — a "gave up on the wallet" capture keeps its high count so it
  // stays on the long TTL instead of oscillating.
  const nextAttempts = force ? 1 : attempts + 1;
  inventoryCache = {
    puuid,
    at: now,
    value,
    attempts: hasChampions(value) && hasWallet(value) ? 0 : nextAttempts,
  };
  return value;
}
